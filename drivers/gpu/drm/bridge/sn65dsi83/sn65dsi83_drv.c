    ...
    struct sn65dsi83 *sn65dsi83)
{
    ...
    sn65dsi83->brg->gpio_reg1v8_enable = devm_gpiod_get(dev, "enable-reg1v8", GPIOD_OUT_LOW);
    if (!IS_ERR(sn65dsi83->brg->gpio_reg1v8_enable)) {
        gpiod_set_value_cansleep(sn65dsi83->brg->gpio_reg1v8_enable, 1);
        msleep(200);
    } else
        dev_warn(dev, "failed to parse enable reg 1v8 gpio");

    sn65dsi83->brg->gpio_reg3v3_enable = devm_gpiod_get(dev, "enable-reg3v3", GPIOD_OUT_LOW);
    if (!IS_ERR(sn65dsi83->brg->gpio_reg3v3_enable)) {
        gpiod_set_value_cansleep(sn65dsi83->brg->gpio_reg3v3_enable, 1);
        msleep(200);
    } else
        dev_warn(dev, "failed to parse enable reg 3v3 gpio");

    sn65dsi83->brg->gpio_reset = devm_gpiod_get(dev, "enable-reset", GPIOD_OUT_LOW);
    if (!IS_ERR(sn65dsi83->brg->gpio_reset)) {
        gpiod_set_value_cansleep(sn65dsi83->brg->gpio_reset, 1);
        msleep(200);
    } else
        dev_warn(dev, "failed to parse enable reset gpio");

    sn65dsi83->brg->gpio_standby = devm_gpiod_get(dev, "enable-standby", GPIOD_OUT_LOW);
    if (!IS_ERR(sn65dsi83->brg->gpio_standby)) {
        gpiod_set_value_cansleep(sn65dsi83->brg->gpio_standby, 1);
        msleep(200);
    } else
        dev_warn(dev, "failed to parse enable standby gpio");
